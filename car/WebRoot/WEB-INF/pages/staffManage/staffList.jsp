...
<link rel="stylesheet" type="text/css" href="${ctx}/js/jBox/Skins-maitian/${themeName}/jbox.css"/>
</head>
    ...
    <div class="bg_color1 border_end "><!--背景色-->
        <div class="pagebody11 wid">
          <div class="pagebody11_top"><!--输入用户名-->
          	<form id="searchStaffForm" name="searchStaffForm" action="${ctx}/staffManage/staffList.shtml" method="post">
                    <ul>
                      <li class="name">输入员工姓名</li>
                      <li ><input type="text" name="username" id="username" class="cha1"  value="${page.t.username}"/></li>
                      ...
                      <table width="1000" border="0"  class="dsfd yuan" >
                          <c:forEach var="staff" items="${page.list}" varStatus="status">
                              <tr>
                                ...
                                <td width="80"><c:if test="${staff.level==2}">普通员工</c:if><c:if test="${staff.level==1}">管理员</c:if></td>
                                <td width="60"><c:if test="${staff.status==0}">在职</c:if><c:if test="${staff.status==1}"><span style="color:red">离职</span></c:if></td>
               ...
               <ul class="page">
<pg:pager url="${ctx}/staffManage/staffList.shtml" items="${page.totalRows}" export="currentPageNumber=pageNumber">
