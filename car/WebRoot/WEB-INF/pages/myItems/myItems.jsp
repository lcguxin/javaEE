...
<script type="text/javascript">
	...
	function mygoods(id) {
		$.jBox("iframe:${ctx}/myItems/toEditMyItems.shtml?id="+id, {
			title : "编辑货架",
			width : 750,
			height : 600,
			buttons : { '关闭': true}
    ...
    function deleteById(id,url){
        ...
        if(!confirm("确定要删除吗?"))
            return;
        var url = "${ctx}/myItems/doDelete.shtml?method=doDelete&id="+id+"&url="+url;
        $.ajax({
            url:url,    //请求的url地址
...
<script type="text/javascript">
	function history() {
		$.jBox("iframe:${ctx}/myItems/myItems.shtml", {
			title : "添加商品",
			width : 750,
			height : 600,
			attach: $('#pagebody1'),
...
<body>
    ...
    <div class="pagebody1" style="height:750px">
    	<div class="wid">
                <ul class="ht1">
                    <li class="big3"><a href="${ctx}/myItems/myItemsList.shtml?method=myItems">我的货架</a></li>
                    <li class="big4"><a href="${ctx}/myItems/myCollection.shtml?method=myCollection">我的收藏</a></li>
        	...
        	<div class="wid2">
                    <ul id="demo">
                        <c:forEach var="sale" items="${uplist}" >
                            <li>
                                <dt onclick="
                                <c:if test="${empty sale.title}">history('${userFlag}');
                                </c:if>
                                <c:if test="${not empty sale.title}">productgroupedit('${sale.id}');
                                </c:if>">
                                <a href="#">
                                <c:if test="${not empty sale.url}">
                                <img src="${sale.url}"/>
                                </c:if>
                                
                                <c:if test="${empty sale.url}"><img src="${ctx}/images/tu007.png"/></c:if>
                                ...
                                <c:if test="${not empty sale.title}">
                                    <dd>
                                            <h3>
                                                <a class="a2" onclick="deleteById('${sale.id}','${sale.url}')"  href="#">&nbsp;&nbsp;&nbsp;&nbsp;</a>
                                            ...
                                            <p><fmt:formatDate value="${sale.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></p>
                                    </dd>
                                </c:if>
                            </li>
                        </c:forEach>
				...
				<div id="List1">
                    ...
                    <ul>
                        <c:forEach var="sale" items="${downlist}" >
                              <li >
                                 <p   class="p1" onclick="<c:if test="${empty sale.title}">history('${userFlag}');</c:if><c:if test="${not empty sale.title}">productgroupedit('${sale.id}');</c:if>"><a href="javascript:;"><c:if test="${not empty sale.url}"><img src="${sale.url}"/></c:if></a>
                                    ...
                                    <c:if test="${empty sale.url}"><img src="${ctx}/images/tu007.png"/></c:if>
                             			...
                             			<c:if test="${not empty sale.title}">
                                      <p class="p2">
                                             <span><a class="a2" onclick="deleteById('${sale.id}','${sale.url}')"  href="#">&nbsp;&nbsp;&nbsp;&nbsp;</a>
                                            <a class="a1" onclick="mygoods('${sale.id}');" href="#">&nbsp;&nbsp;&nbsp;&nbsp;</a>
                                            <span title="${sale.title}" style="cursor:pointer;">
                                            ...
                                            <c:choose>
                                             ...
                                             <b><a><fmt:formatDate value="${sale.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></a></b>
                                     </p>
                                	</c:if>
                             </li>
                        </c:forEach>
